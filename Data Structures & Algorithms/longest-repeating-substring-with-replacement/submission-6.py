class Solution:
    def characterReplacement(self, s: str, k: int) -> int:
        ...
        for r in range(len(s)):
            ...
            if (r-l+1) - max(count.values())>k: #r-l+1 is the length of the current window 
                count[s[l]]-=1
                l+=1
            res=max(res, r-l+1)
        return res
...
#we wanna find the longest substring, known that we can replace up to k element 
# we might use sliding window, left pointer gonna start at the start, t
#he right ptr gonna run from 0->len(s)-1, it might be break when k reach its limit

# when are we gonna move the pointer:
#   
#   
