...
idea: we will move the window until its no longer valid, in this case: the number of replacement exceed

every time looping, we will extend the window => we create a hashmap to track the occurance of the char inside the number
we find the max_occurance in the hasmap value by checking: max(longest, hashmap[s[r]])

we will need to check if the current window is valid or not by using: len of window - max_occurance >k: 
if not, decrease the occurance of the var that left ptr point to and move the left ptr
if nothing is wrong, then track the longest
"""
...
def longest_repeating(s,k):
    ...
    for r in range(len(s)):
        ...
        #WE NEED TO CHECK IF THE CURRENT WINDOW IS VALID OR NOT 
        if r-l+1 - max_occurance>k:
            hashmap[s[l]]-=1
            l+=1
        
        #we also need to track the length of the s
        longest=max(longest, r-l+1)
