...
class Solution:  
    ...
    def isSubtree(self, root: Optional[TreeNode], subRoot: Optional[TreeNode]) -> bool:
        ...
        def is_same_tree(root, subroot):
            ...
            left_tree= is_same_tree(root.left, subroot.left)
            right_tree= is_same_tree(root.right, subroot.right)

            return left_tree and right_tree 
        

        #search the root and determine if its the same as given subroot
        #return a bool
        def search_subtree(root):
            ...
            #when there exists a root, if it is a valid one => return true
            if is_same_tree(root, subRoot):
                return True
            
            #if it is not a valid one => search left and right
            left_subtree= search_subtree(root.left)
            right_subtree= search_subtree(root.right)

            #if 1 out of 2 subtree match => true
