class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # if the closest car reach the destination faster than the prev => add it to the stack to represent a fleet 
        # else if it is slower => remove the faster car (since faster car will merge with slower one) and keep compare to the prev


        cars=[]

        #create car and add it to the list "cars"
        for i in range(len(position)):
            car = [position[i],speed[i]]
            cars.append(car)
        
        #sort desending by position
        ...
        for car in cars:
            ...
            #pop when they reach the destination at the same time or the car behind
            #meet the first car before getting to the destination
