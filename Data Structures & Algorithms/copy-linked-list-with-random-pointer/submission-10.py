...
ideas:
    hashmap={None: None}
        + key: the node with ptr (the og node)
        + value: the node with value only

    we itertate through the linkedlist to store the pair of real node and copy node
    Iterate again to assign the pointer
"""
...
def copy_1(head):
    ...
    #iterate through linkedlist to assign (node : copy_node) pair to hashmap
    while curr:
        copy_node = Node(curr.val, None, None)
        hashmap[curr] = copy_node
        curr= curr.next
