...
def meeting_room_2(intervals):
    ...
    # min heap (or priority queue) will be used to store the ending time of every interval
    # it always prioritize the early-ended interval 
    pq = []
    heapq.heapify(pq)
    ...
    for i in range(1, len(intervals)):
        if pq and intervals[i].start >= pq[0]:
            heapq.heappop(pq)
